class Lander {
  ...
  metaBuyers() {
    ...
    if (!localStorage.getItem('lands-meta-c2')) {
      let meta = {
        buyers: 87,
        visits: 2130,
        now: 34,
      ...
      localStorage.setItem('lands-meta-c2', JSON.stringify(meta))
    }
    ...
    function SetMetaData() {
      let data = JSON.parse(localStorage.getItem('lands-meta-c2'))
    ...
    function updateShop() {
      ...
      function commentsShow() {
        ...
        if (names.length > 1) {
          commentInnerText.innerHTML = `${names[random]}, Συνολικό ποσο παραγγελίας ${price}, 1 τμχ.`;
      ...
      window.addEventListener('scroll', (e) => {
        ...
        if (!data.firstShowOnOffer) {
          if (document.documentElement.scrollTop + 300 > offer.offsetTop) {
            ...
            localStorage.setItem('lands-meta-c2', JSON.stringify(data))
            commentsShow()
          }
        }
      })
...
const lander = new Lander({
  ...
  names: [
    ...
    'Γιάννης, Βολος',
    'Αλέξης, Λαμία',
    'Αλεξάνδρα, Θεσσαλονίκη',
    'Θεοδωρα, Πύργος',
    'Μανώλης, Μεσσηνία ',
    'Κατερίνα, Σαντορίνη',
    'Αλέξανδρος, Αλεξανδρούπολη'
  ],
  commentDuration: 6
}).init()
