class Lander {
  ...
  metaBuyers() {
    ...
    if (!localStorage.getItem('lands-meta-p')) {
      let meta = {
        buyers: 87,
        visits: 2130,
        now: 34,
      ...
      localStorage.setItem('lands-meta-p', JSON.stringify(meta))
    }
    ...
    function SetMetaData() {
      let data = JSON.parse(localStorage.getItem('lands-meta-p'))
      ...
      window.addEventListener('scroll', (e) => {
        ...
        if (!data.firstShowOnOffer) {
          if (document.documentElement.scrollTop + 300 > offer.offsetTop) {
            ...
            localStorage.setItem('lands-meta-p', JSON.stringify(data))
            commentsShow()
          }
        }
      })
